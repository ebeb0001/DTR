...
\begin{table}[h]
    ...
    \begin{tabular}{lccc}
        ...
        Number of generated train routes & 49 \cr
        Number of complete simulated trips & 49 \cr
        \bottomrule
    \end{tabular}
...
The micro-level pipeline is expected to require more processing time. This is mainly due to the larger 
number of infrastructure elements and to the additional steps required to obtain a simulation-ready network. In particular, 
switch detection, platform matching, and routing through the detailed track graph introduce additional computational 
complexity. These steps are necessary to obtain a more realistic infrastructure representation, but they also make the 
generation process more expensive.
...
\begin{table}[h]
    ...
    \begin{tabular}{lc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Execution time of the macro-level generation pipeline for the example network}\label{tab:pipeline_performance}
\end{table}
...
\begin{table}[h]
    ...
    \begin{tabular}{lc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Execution time of the micro-level generation pipeline with random trip generation}\label{tab:pipeline_performance}
\end{table}
...
Overall, this performance analysis provides a practical view of the scalability of the proposed framework. It shows 
not only whether the pipeline can generate valid simulation scenarios, but also how expensive this generation is and 
which components may become bottlenecks when the network size or simulation period increases.
