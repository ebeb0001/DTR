...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        Macro-Level & 1327 & 1337 & 98.80 \cr
...
The results show that station coverage is very high for both modeling approaches. For the macro-level simulation, 1327 
stations are generated from a reference dataset containing 1337 stations, corresponding to a coverage of 98.80\%. For the 
micro-level simulation, 553 stations are generated from 554 reference stations, corresponding to a coverage of 99.82\%.

These results indicate that the station extraction process is able to recover almost all stations present in the reference 
data. The small difference between the reference and generated datasets can be explained by inconsistencies in the 
available data, filtering steps applied during preprocessing, or stations for which no valid infrastructure association 
could be established. Nevertheless, the high coverage obtained in both cases confirms that the generated datasets provide 
a sufficiently complete station basis for the rest of the modeling pipeline.
...
Stations are almost completely detected in both modeling approaches, and platforms are also recovered with a high 
success rate in the micro-level model.
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        Stations (Macro-Level) & 1327 & 1337 & 99.85 \cr
        Stations (Micro-Level) & 553 & 554 & 99.82 \cr
        Platforms (Micro-Level) & 1551 & 1560 & 99.42 \cr
        Switches (Micro-Level) & 12343 & 10379 & 118.92 \cr
        \bottomrule
    \end{tabular}
...
The detection percentage reaches 99.85\% for macro-level stations and 99.82\% for micro-level stations. Platform detection 
reaches 99.42\%, which confirms that most station platforms could be matched to the generated infrastructure. These results 
are important because platforms are used as stopping locations for trains and as origin or destination points during trip 
generation.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{chapters/images/switch_detection_before.png}
    \caption{Network before and after switch detection}\label{fig:switch_detection_before}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{chapters/images/switch_detection_after.png}
    \caption{Network before and after switch detection}\label{fig:switch_detection_after}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{chapters/images/platform_matching_result.png}
    \caption{Platform matching result on Bruxelles-Midi}\label{fig:platform_matching}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{chapters/images/platform_detection_quality.png}
